...
#    print lon1
    clm=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamhistorical_cru/heat/fertfao/fixedirr_new1_n/maizetemp_historical_co2_rf_fert_0.5x0.5.nc','r')
    clmtropf = clm.variables['total_gfna'][bb,:,:]
    clmtropf= ma.masked_where(clmtropf<=0,clmtropf)
    clmtropf= ma.masked_where(maizeto<=0,clmtropf)
    clmtropf[N.isinf(clmtropf)]=-1
    clmtropf[N.isnan(clmtropf)]=-1
    clmtropf=ma.filled(clmtropf, fill_value=0.)
    clmtropf= ma.masked_where(clmtropf<=0,clmtropf)

    clm2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamhistorical_cru/heat/fertfao/fixedirr_new1_n/maizetemp_historical_co2_irrig_fert_0.5x0.5.nc','r')
    clmtropf1 = clm2.variables['total_gfna'][bb,:,:]
    clmtropf1= ma.masked_where(clmtropf1<=0,clmtropf1)
    clmtropf1= ma.masked_where(maizeto<=0,clmtropf1)
    clmtropf1[N.isinf(clmtropf1)]=-1
    clmtropf1[N.isnan(clmtropf1)]=-1
    clmtropf1=ma.filled(clmtropf1, fill_value=0.)
    clmtropf1= ma.masked_where(clmtropf1<=0,clmtropf1)



    clm3n=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamhistorical_cru/heat/fertfao/fixedirr_new1_n/maizetemp_historical_co2_rf_nofert_0.5x0.5.nc','r')
    isammai = clm3n.variables['total_gfna'][bb,:,:]
    isammai= ma.masked_where(isammai<=0,isammai)
    isammai= ma.masked_where(maizeto<=0,isammai)
    isammai[N.isinf(isammai)]=-1
    isammai[N.isnan(isammai)]=-1
    ...
    isamcrumai= ma.masked_where(maizeto<=0,isamcrumai)
    isamcrumai=ma.filled(isamcrumai, fill_value=0.)

    clm3n2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamhistorical_cru/heat/fertfao/fixedirr_new1_n/maizetemp_historical_co2_irrig_nofert_0.5x0.5.nc','r')
    isamcrumai2 = clm3n2.variables['total_gfna'][aa,:,:]
    isamcrumai2= ma.masked_where(isamcrumai2<=0,isamcrumai2)
    isamcrumai2= ma.masked_where(maizeto<=0,isamcrumai2)
    isamcrumai2[N.isinf(isamcrumai2)]=-1
    isamcrumai2[N.isnan(isamcrumai2)]=-1
    ...
    isamcrumai3= ma.masked_where(maizeto<=0,isamcrumai3)
...
#plt.xticks(xdates, xdates)
        ax.plot_date(xdates1,faoy,"ro-",linewidth=2)

        ax.plot_date(xdates,isamy2,"ko-",label="NO-NF")
        ax.plot_date(xdates,clmy1,"bo-",label="Base_notill")
        ax.plot_date(xdates,clmy,"go-",label="NO-I_notill")
        ax.plot_date(xdates,isamy,"yo-",label="NO-I & NF")
	...
	plt.savefig('maize_faoclm_{0}_1961_2016_n.png'.format(name1),dpi=300,bbox_inches='tight')
plt.show()
