...
def annualyield(year,couna,counb):
    ...
    clm3n2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/plot/finalyield/isam/heat/fertfao/isamhis_soyscaleiyield_heat_fertfao.nc','r')
    ...
    return harea, yieldc,yieldi,yieldic,yieldclm,yieldisam,yieldisamc,yieldic2,yieldic3,yieldisamc2,yieldisamc3
    #return harea
def runmean(x,input):
        import pandas as pd
...
#plt.xticks(xdates, xdates)
	...
	ccp2=scipy.stats.pearsonr(clmy,faoy)
        ...
        leg=plt.legend(['FAO', 'Nfix {:04.3f}'.format(ccp[0]),'NScale {:04.2f}'.format(ccp1[0]),'NFao {:04.2f}'.format(ccp3[0]),'NAll {:04.2f}'.format(ccp4[0]),'CLM {:04.2f}'.format(ccp2[0])],fontsize=18)
...
#leg = plt.legend(loc=2,fancybox=True, fontsize=14)
	...
	plt.savefig('soy_faon_{0}.png'.format(name1),dpi=600,bbox_inches='tight')
plt.show()
