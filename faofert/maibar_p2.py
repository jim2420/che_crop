...
bar_width = 0.18
opacity = 0.8
rects0 = plt.bar(index, finalnew, bar_width,
         alpha=opacity,color='gray',
         label='Global')
rects1 = plt.bar(index+bar_width, finalnewi, bar_width,
         alpha=opacity,color='green',
         label='Trop. irrigated')
rects2 = plt.bar(index+bar_width*2, finalnewr, bar_width, 
         alpha=opacity,color='lime',
         label='Trop. rainfed')
rects1 = plt.bar(index+bar_width*3, finalnewie, bar_width, 
         alpha=opacity,color='blue',
         label='Temp. irrigated')
rects2 = plt.bar(index+bar_width*4, finalnewre, bar_width,
         alpha=opacity,color='cyan',
         label='Temp. rainfed')


plt.ylabel('Percentage of maize yield change',fontsize=18)
plt.tick_params(axis='both',labelsize=18)
plt.xticks(index + bar_width+0.25, ('Irrigation','N fertilizer','CO$_{2}$','Climate'))
leg=plt.legend(loc=1)
leg.get_frame().set_alpha(0.5)

plt.tight_layout()

plt.savefig('maize_his_irrfixed_paper3.png')
plt.show()
