...
def annualyield(year,couna,counb):
    ...
    clm2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/plot/finalyield/isam/heat/fertfao/new1/isamhiscru_maiscaleiyield_fertfao_new1_long_notill.nc','r')
...
#plt.xticks(xdates, xdates)
	...
	ax.plot_date(xdates1,faoy,"ro-",label="FAO",linewidth=2)
        ...
        ax.plot_date(xdates,clmy1,"bo-",label="ISAM-notill")
	...
	plt.tick_params(axis='both',labelsize=18)
        ...
        ax.plot_date(xdates1,faoy/(N.average(faoy[40:50])),"ro-",label="FAO",linewidth=2)
        ax.plot_date(xdates,isamy2/(N.average(isamy2[40:50])),"ko-",label="ISAM")
        ax.plot_date(xdates,clmy1/(N.average(clmy1[40:50])),"bo-",label="ISAM-notill")
        ax.plot_date(xdates,clmy/(N.average(clmy[40:50])),"go-",label="CLM-Scale")
        ...
        plt.xlabel("Year",fontsize=18)
        plt.ylabel("Relative maize yield (t/ha)2001-2010",fontsize=18)
        plt.title("Yield",fontsize=18)
        plt.tick_params(axis='both',labelsize=18)



	plt.savefig('maize_faoclm_{0}_1961_2016_a2.png'.format(name1),dpi=600,bbox_inches='tight')
plt.show()
