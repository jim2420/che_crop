...
country=NetCDFFile('/scratch2/scratchdirs/tslin2/plot/globalcrop/data/surfdata_05x05_13reg.nc','r')
coun = country.variables['REGION_MASK_CRU_NCEP'][:,:]
latr=country.variables['lat'][:]
lonr=country.variables['lon'][:]
coun,lonr = shiftgrid(180.5,coun,lonr,start=False)
#print lonr
...
def annualyield(year,couna,counb):
    ...
    base = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetemp_rcp85_constco2_rf_nofert_0.5x0.5.nc", mode='r')
    base2 = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetemp_rcp85_co2_rf_nofert_0.5x0.5.nc", mode='r')
    ...
    basei = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetemp_rcp85_co2_rf_fert_0.5x0.5.nc", mode='r')
    base2i = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetemp_rcp85_co2_irrig_fert_0.5x0.5.nc", mode='r')

    yieldfi = basei.variables["totalyield"][bb,:,:]
        
    yieldfai = base2i.variables["totalyield"][bb,:,:]
       
    baseif = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetrop_rcp85_co2_rf_fert_0.5x0.5.nc", mode='r')
    base2if = NetCDFFile ("/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamrcp85/heat/maizetrop_rcp85_co2_irrig_fert_0.5x0.5.nc", mode='r')
...
name=["USA","North America","Latin America","European Union","Sub-Sah Africa","USSR","China","India","South and Southeast Asia","Pacific Developed","North Africa"]
range1=[12,1,2,3,5,6,7,13,8,9,4]
range2=[12,1,2,3,5,6,7,13,8,9,4]
...
rects01 = plt.bar(index+bar_width*4, fisamc1, bar_width,
                 alpha=opacity,
                 color='b',
                 label='ISAM-C')
rects41 = plt.bar(index+bar_width*5, fclmc1, bar_width,hatch=".",
                 alpha=opacity,
                 color='b',
                 label='CLM-C')
rects02 = plt.bar(index+bar_width*6, fisamd1, bar_width,
                 alpha=opacity,
                 color='g',
                 label='ISAM-D')
rects43 = plt.bar(index+bar_width*7, fclmd1, bar_width,bar_width,hatch=".",
                 alpha=opacity,
                 color='g',
                 label='CLM-D')
...
plt.xticks(index + bar_width+0.4, ("USA","North America","Latin America","European Union","Sub-Sah Africa","USSR","China","India","South and Southeast Asia","Pacific Developed","North Africa"),rotation='vertical')
leg=plt.legend(fontsize=40,loc=4)
leg.get_frame().set_alpha(0.5)
plt.tick_params(axis='both',labelsize=40)
plt.tight_layout()

plt.savefig('maize2050_85_reg.png')
#plt.show()
...
ax.yaxis.set_ticks(N.arange(-85,85,20))
ax.xaxis.set_ticks(N.arange(-85,85,20))

ccp1=scipy.stats.pearsonr(fisama1,fclma1)
leg=ax.legend(['1:1', 'CLIMATE {:04.2f}'.format(ccp1[0])],fontsize=12)

plt.xlim(-85, 85)
plt.ylim(-85, 85)
leg.get_frame().set_alpha(0.5)

n=["US","NA","LA","EU","SSA","USSR","C","I","SSEA","PD","NAF"]
for i, txt in enumerate(n):
    ax.annotate(txt, (fisama1[i],fclma1[i]))

plt.tick_params(axis='both',labelsize=18)
plt.xlabel('ISAM',fontsize=18)
plt.ylabel('CLM',fontsize=18)
...
colorsr = (0,0,1)
color=(0,1,0)
ax1.plot([-100,100],[-100,100], 'k--',label='1:1')
ax1.scatter(fisamb1, fclmb1,s=124, c=color,alpha=0.5,label='CLIMATE+CO2')
ax1.yaxis.set_ticks(N.arange(-85,85,20))
ax1.xaxis.set_ticks(N.arange(-85,85,20))

ccp2=scipy.stats.pearsonr(fisamb1,fclmb1)

leg1=ax1.legend(['1:1', 'CLIMATE+CO2 {:04.2f}'.format(ccp2[0])],fontsize=12)

n=["US","NA","LA","EU","SSA","USSR","C","I","SSEA","PD","NAF"]
for i, txt in enumerate(n):
    ax1.annotate(txt, (fisamb1[i],fclmb1[i]))
plt.xlim(-85, 85)
plt.ylim(-85, 85)
...
colorsr = (0,0,1)
color=(0,1,0)
ax2.plot([-100,100],[-100,100], 'k--',label='1:1')
ax2.scatter(fisamc1, fclmc1,s=124, c='blue',alpha=0.5,label='CLIMATE+CO2+N')
ax2.yaxis.set_ticks(N.arange(-85,85,20))
ax2.xaxis.set_ticks(N.arange(-85,85,20))

ccp2=scipy.stats.pearsonr(fisamc1,fclmc1)

leg2=ax2.legend(['1:1', 'CLIMATE+CO2+N {:04.2f}'.format(ccp2[0])],fontsize=12)
...
n=["US","NA","LA","EU","SSA","USSR","C","I","SSEA","PD","NAF"]
for i, txt in enumerate(n):
    ax2.annotate(txt, (fisamc1[i],fclmc1[i]))
...
colorsr = (0,0,1)
color=(0,1,0)
ax3.plot([-100,100],[-100,100], 'k--',label='1:1')
ax3.scatter(fisamd1, fclmd1,s=124, c='red',alpha=0.5,label='CLIMATE+CO2+N+I')
ax3.yaxis.set_ticks(N.arange(-85,85,20))
ax3.xaxis.set_ticks(N.arange(-85,85,20))

ccp2=scipy.stats.pearsonr(fisamd1,fclmd1)

leg3=ax3.legend(['1:1', 'CLIMATE+CO2+N+I {:04.2f}'.format(ccp2[0])],fontsize=12)

n=["US","NA","LA","EU","SSA","USSR","C","I","SSEA","PD","NAF"]
for i, txt in enumerate(n):
    ax3.annotate(txt, (fisamd1[i],fclmd1[i]))
plt.xlim(-85, 85)
plt.ylim(-85, 85)
...
plt.savefig('scatter_mai85_reg1.png',bbox_inches='tight')
