...
#        		else:
#      				fclmd[xx,yy]=0.0
#        fclmb=N.zeros((360,720))
#        for xx in range(0,360):
#                for yy in range(0,720):
#                        if clmb10[xx,yy]>0:
#                                fclmb[xx,yy]=(clmb50[xx,yy]-clmb10[xx,yy])/clmb10[xx,yy]*100.
#                        else:
#                                fclmb[xx,yy]=0.0
#        fclmb=ma.masked_where(fclmb==0,fclmb)
#        fclmd=ma.masked_where(fclmd==0,fclmd) 
        fclmb=ma.masked_where(fclmb>=5000,fclmb)
        fclmc=ma.masked_where(fclmc>=5000,fclmc)
        fclmd=ma.masked_where(fclmd>=5000,fclmd)
        fclma=ma.masked_where(fclma>=5000,fclma)
        fisamb=ma.masked_where(fisamb>=5000,fisamb)
        fisamc=ma.masked_where(fisamc>=5000,fisamc)
        fisamd=ma.masked_where(fisamd>=5000,fisamd)
        fisama=ma.masked_where(fisama>=5000,fisama)
...
#	print ac
	bplot=plt.boxplot(ac,labels=lab,showfliers=False,patch_artist=True,showmeans=True)
        ...
        plt.title('2050s-2010s maize',fontsize=16)
...
#	plt.figtext(0.815, 0.013, ' CO2', color='black', weight='roman',size='x-small')
	...
	plt.tight_layout()
        ...
        plt.savefig('maize2050_85box.png')
	plt.show()
