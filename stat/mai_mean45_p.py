...
for i, name1 in enumerate(name):
	...
	for num in range(a1,a2):
        ...
        c1=2050-2010
        c2=2060-2010
	isamya = ma.masked_where(isamya<=0,isamya)
        clmya = ma.masked_where(clmya<=0,clmya)
        isamyb = ma.masked_where(isamyb<=0,isamyb)
        clmyb = ma.masked_where(clmyb<=0,clmyb)
        isamyc = ma.masked_where(isamyc<=0,isamyc)
        clmyc = ma.masked_where(clmyc<=0,clmyc)
        isamyd = ma.masked_where(isamyd<=0,isamyd)
        clmyd = ma.masked_where(clmyd<=0,clmyd)
	...
	fisama=(isama50-isama10)/isama10*100.
        ...
        isamya=ma.filled(isamya, fill_value=-999)
        isamyb=ma.filled(isamyb, fill_value=-999)
        isamyc=ma.filled(isamyc, fill_value=-999)
        isamyd=ma.filled(isamyd, fill_value=-999)

        clmya=ma.filled(clmya, fill_value=-999)
        clmyb=ma.filled(clmyb, fill_value=-999)
        clmyc=ma.filled(clmyc, fill_value=-999)
        clmyd=ma.filled(clmyd, fill_value=-999)

        isaaold=list(isamya[d1:d2,:,:].flat)
        clmaold=list(clmya[d1:d2,:,:].flat)
        isabold=list(isamyb[d1:d2,:,:].flat)
        clmbold=list(clmyb[d1:d2,:,:].flat)
        isacold=list(isamyc[d1:d2,:,:].flat)
        clmcold=list(clmyc[d1:d2,:,:].flat)
        isadold=list(isamyd[d1:d2,:,:].flat)
        clmdold=list(clmyd[d1:d2,:,:].flat)

        isaanew=list(isamya[c1:c2,:,:].flat)
        clmanew=list(clmya[c1:c2,:,:].flat)
        isabnew=list(isamyb[c1:c2,:,:].flat)
        clmbnew=list(clmyb[c1:c2,:,:].flat)
        isacnew=list(isamyc[c1:c2,:,:].flat)
        clmcnew=list(clmyc[c1:c2,:,:].flat)
        isadnew=list(isamyd[c1:c2,:,:].flat)
        clmdnew=list(clmyd[c1:c2,:,:].flat)


        isaaold = [x for x in isaaold if x != -999]
        isabold = [x for x in isabold if x != -999]
        isacold = [x for x in isacold if x != -999]
        isadold = [x for x in isadold if x != -999]
        isaanew = [x for x in isaanew if x != -999]
        isabnew = [x for x in isabnew if x != -999]
        isacnew = [x for x in isacnew if x != -999]
        isadnew = [x for x in isadnew if x != -999]

        clmaold = [x for x in clmaold if x != -999]
        clmbold = [x for x in clmbold if x != -999]
        clmcold = [x for x in clmcold if x != -999]
        clmdold = [x for x in clmdold if x != -999]
        clmanew = [x for x in clmanew if x != -999]
        clmbnew = [x for x in clmbnew if x != -999]
        clmcnew = [x for x in clmcnew if x != -999]
        clmdnew = [x for x in clmdnew if x != -999]

	#print isaaold
	itca, ipTca = ttest_ind(isaaold,isaanew, axis=0 ,equal_var = False)
        itcb, ipTcb = ttest_ind(isabold,isabnew, axis = 0, equal_var = False)
        itcc, ipTcc = ttest_ind(isacold,isacnew, axis = 0, equal_var = False)
        itcd, ipTcd = ttest_ind(isadold,isadnew, axis = 0, equal_var = False)
	print itca,itcb,itcc,itcd,ipTca,ipTcb,ipTcc,ipTcd
        citca, cipTca = ttest_ind(clmaold,clmanew, axis = 0, equal_var = False)
        citcb, cipTcb = ttest_ind(clmbold,clmbnew, axis = 0, equal_var = False)
        citcc, cipTcc = ttest_ind(clmcold,clmcnew, axis = 0, equal_var = False)
        citcd, cipTcd = ttest_ind(clmdold,clmdnew, axis = 0, equal_var = False)
        print citca,citcb,citcc,citcd,cipTca,cipTcb,cipTcc,cipTcd
	...
	plt.ylim(-40,10)
        ...
        for i,p in enumerate(pvalsa):
            if p>=0.05:
                ...
                displaystring = r'*'
	    if fisama[i]<0:
		height = -offset +  min(fisama[i],fisama[i])
	    else:
	        height = offset +  max(fisama[i],fisama[i])
            bar_centers = index[i] + N.array([0.0,1.0])*bar_width
            significance_bar(bar_centers[0],bar_centers[1],height,displaystring)

        offset  =2
        for i,p in enumerate(pvalsb):
            if p>=0.05:
                displaystring = r'n.s.'
            elif p<0.0001:
                displaystring = r'***'
                ...
                displaystring = r'*'
            if fisama[i]<0:
                height = -offset +  min(fisamb[i],fisamb[i])
            else:
                height = offset +  max(fisamb[i],fisamb[i])
            bar_centers = index[i] + N.array([1.0,2.0])*bar_width
            significance_bar(bar_centers[0],bar_centers[1],height,displaystring)
        offset  =2
        for i,p in enumerate(pvalsc):
            if p>=0.05:
                displaystring = r'n.s.'
            elif p<0.0001:
                displaystring = r'***'
                ...
                displaystring = r'*'
            if fisama[i]<0:
                height = -offset +  min(fisamc[i],fisamc[i])
            else:
                height = offset +  max(fisamc[i],fisamc[i])
            bar_centers = index[i] + N.array([2.0,3.0])*bar_width
            significance_bar(bar_centers[0],bar_centers[1],height,displaystring)
        offset  =2
        for i,p in enumerate(pvalsd):
            if p>=0.05:
                displaystring = r'n.s.'
            elif p<0.0001:
                displaystring = r'***'
                ...
                displaystring = r'*'
            if fisama[i]<0:
                height = -offset +  min(fisamd[i],fisamd[i])
            else:
                height = offset +  max(fisamd[i],fisamd[i])
            bar_centers = index[i] + N.array([3.0,4.0])*bar_width
            significance_bar(bar_centers[0],bar_centers[1],height,displaystring)
...
#	plt.title('2050s-2010s maize',fontsize=18)
	...
	plt.savefig('maize2050_45_new1.png')
	plt.show()
