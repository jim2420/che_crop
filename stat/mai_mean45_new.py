...
#    Return the weighted average and standard deviation. values, weights -- Numpy ndarrays with the same shape.
	average1=N.average(values,weights=weights)
	variance=N.average((values-average1)**2,weights=weights)  # Fast and numerically precise
...
#        fisamd=(isamd50-isamd10)/isamd10*100.
        ...
        fclmc=(clmc50-clmc10)/clmc10*100.
        fclmd=(clmd50-clmd10)/clmd10*100.
	fisamd=N.zeros((360,720))
	for xx in range(0,360):
        	for yy in range(0,720):
            		if frac50a[xx,yy]>0:
				fisamd[xx,yy]=(isamd50[xx,yy]-isamd10[xx,yy])/isamd10[xx,yy]*100.
			else:
				fisamd[xx,yy]=-999.0


	fisama1=N.isnan(fisamd)
	fisamd[fisama1]=-999.0
        fisama1=N.isinf(fisamd)
        fisamd[fisama1]=-999.0
	fisamd = ma.masked_where(fisamd==-999.0,fisamd)
        fisamd = ma.masked_where(fisamd==0.0,fisamd)

	print fisamd.shape

        
	aasn=weighted_avg_and_std(fisamd,frac50a)
	print aasn
